...
spotify_songs %>% 
  mutate(release_date = as_date(track_album_release_date),
         release_year = ifelse(is.na(release_date), as.numeric(track_album_release_date), year(release_date))) %>% 
  filter(is.na(release_year)) %>% 
  select(track_album_release_date, release_year)


spotify_songs <- spotify_songs %>% 
  mutate(release_year = str_sub(track_album_release_date, 1, 4),
...
spotify_songs %>% 
  ...
  geom_line() +
  geom_hline(yintercept = 200, col = "darkred")

start_year <- 1995

spotify_songs %>% 
  filter(release_year >= start_year) %>% 
  group_by(release_year) %>% 
  summarize(valence_mean = mean(valence)) %>% 
  ggplot(aes(release_year, valence_mean)) +
  geom_line() +
